...
# ---------------------------------------------------------------------------
# Các dependency dưới đây (aioboto3, torch, transformers, openai) chỉ thật sự
# cần khi dùng backend tương ứng (Bedrock, HuggingFace local model, OpenAI/Azure).
# Import kiểu "lazy/optional" để bản dùng Ollama-only (chạy nội bộ, không API)
# không bắt buộc phải cài PyTorch/transformers/boto3/openai (nặng, không cần thiết).
# ---------------------------------------------------------------------------
try:
    import aioboto3
except ImportError:
